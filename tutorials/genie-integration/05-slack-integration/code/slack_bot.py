...
This bot demonstrates the M+N pattern:
- Uses shared/mcp_client.py (same code as Teams bot!)
- Thin wrapper around universal MCP client
...
def format_genie_response(raw_response: str) -> str:
    ...
    Genie MCP returns nested JSON that needs to be unpacked.
    """
    try:
