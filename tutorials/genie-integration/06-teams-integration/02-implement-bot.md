...
Write the Teams bot code that connects to Genie via MCP.

## Project Structure

```
tutorials/genie-integration/06-teams-integration/
├── code/
│   ├── teams_bot.py          # Main bot code
...
# Teams (leave empty for local testing with Agents Playground)
MICROSOFT_APP_ID=
MICROSOFT_APP_PASSWORD=
```

**Note:** For Agents Playground, you don't need Azure credentials!
...
1. **MCP Client:**
   ...
   mcp_client = create_mcp_client()
   ```
   Same client as Slack bot - demonstrates code reuse!

2. **Bot Framework Adapter:**
   ```python
   ADAPTER = BotFrameworkAdapter(SETTINGS)
   ```
   Handles Teams protocol communication.

3. **Message Handler:**
   ```python
   async def on_message_activity(turn_context: TurnContext):
   ```
   Processes incoming messages and queries Genie.
...
4. **Genie Query:**
   ...
   ```
   Single method call - same as Slack!
...
- [Test with Playground](03-test-with-playground.md) - Test your bot
- [Production Deployment](04-production-deployment.md) - Deploy to Azure (when ready)
